...
> **Windows note:** On Windows the snapshot directory is hardened with an ACL (via `icacls`) on a best-effort basis. Since snapshots contain plaintext secrets like `.env` / `auth.json`, double-check that the directory is only accessible by your own account.
...
├── presets/               # Preset templates (hermes / openclaw / claude-code / gemini / codex)
├── tests/                 # Test suite
